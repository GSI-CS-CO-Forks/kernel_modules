...
int  i, fd;
   ...
   for (i = 1; i <= CtrDrvrCLIENT_CONTEXTS; i++) {
      sprintf(fnm,"/dev/%s.%1d",devnames[ctr_device],i);
      if ((fd = open(fnm,O_RDWR,0)) > 0) break;
   }
   unlock_lib();
   return fd;
}
...
static uint32_t      CtrHptdcToNano(uint32_t      hptdc) {
double fns;
uint32_t      ins;
...
uint32_t      CtrLibGetInstalledModuleCount() {
...
char cmd[64];
int mods;
uint32_t      p2c, oby; /* VME P2Connector output byte */
...
TimLibError CtrLibConnect(TimLibClass   iclss,    /* Class of interrupt */
			  uint32_t      equip,    /* Equipment or hardware mask */
			  uint32_t      module) { /* For HARD or CTIM classes */
...
CtrDrvrConnection con;
   ...
   if (con.EqpNum == 0) {
      if (ioctl(fd,CtrIoctlDISCONNECT,&con) < 0) return TimLibErrorIO;
      ctr_connected = 0;
      return TimLibErrorSUCCESS;
   }
   if (ioctl(fd,CtrIoctlCONNECT,&con) < 0) return TimLibErrorCONNECT;
...
TimLibError CtrLibConnectPayload(uint32_t      ctim,        /* The CTIM ID you want to connect to */
				 uint32_t      payload,     /* The 16 bit payload in a long */
				 uint32_t      module) {    /* The module, or zero means don't care */
CtrDrvrAction act;
...
TimLibError CtrLibQueue(uint32_t      qflag,    /* 0=>Queue, 1=>NoQueue  */
			uint32_t      tmout) {  /* 0=>No time outs       */
...
TimLibError CtrLibFdQueue(int           fd,      /* File descriptor       */
			  uint32_t      qflag,   /* 0=>Queue, 1=>NoQueue  */
			  uint32_t      tmout) { /* 0=>No time outs       */
   ...
   if (ioctl(fd,CtrIoctlSET_TIMEOUT,&tm)    < 0) return TimLibErrorIO;
   if (ioctl(fd,CtrIoctlSET_QUEUE_FLAG,&qf) < 0) return TimLibErrorIO;
...
TimLibError CtrLibWait(TimLibClass    *iclss,      /* Class of interrupt */
		       uint32_t       *equip,      /* PTIM CTIM or hardware mask */
		       uint32_t       *plnum,      /* Ptim line number 1..n or 0 */
		       ...
		       uint32_t       *ctim,       /* CTIM trigger equipment ID */
		       uint32_t       *payload,    /* Payload of trigger event */
		       uint32_t       *module,     /* Module that interrupted */
		       uint32_t       *missed,     /* Number of missed interrupts */
		       uint32_t       *qsize,      /* Remaining interrupts on queue */
...
TgmMachine         mch;
   ...
   while (1) {
      ...
      if (rbf.Connection.EqpClass == CtrDrvrConnectionClassPTIM) {
         uint32_t m = rbf.Connection.Module;
	 ioctl(fd,CtrIoctlSET_MODULE,&m);
	 act.TriggerNumber = rbf.TriggerNumber;
	 if (ioctl(fd,CtrIoctlGET_ACTION,&act) < 0) return TimLibErrorIO;
	 if ((act.Config.OnZero & CtrDrvrCounterOnZeroOUT) == 0) continue;
      }
      break;
   }
   
   if (cbl == 0) ioctl(fd,CtrIoctlGET_CABLE_ID,&cbl);
   mch = TgvTgvToTgmMachine(TgvFirstMachineForCableId(cbl));
   ...
   if (plnum) {
      if (rbf.Connection.EqpClass == CtrDrvrConnectionClassPTIM) {
	 ob.EqpNum = rbf.Connection.EqpNum;
	 ioctl(fd,CtrIoctlGET_PTIM_BINDING,&ob);
	 *plnum  = rbf.TriggerNumber - ob.StartIndex;
      } else *plnum = 0;
   }
   if (missed) {
	ioctl(fd,CtrIoctlGET_QUEUE_OVERFLOW,&miss);
	*missed = miss;
   }
   if (qsize) {
	ioctl(fd,CtrIoctlGET_QUEUE_SIZE,&qs);
	*qsize = qs;
   }
...
TimLibError CtrLibSet(uint32_t      ptim,    /* PTIM to write to */
		      uint32_t      plnum,   /* Ptim line number 1..n or 0 */
		      uint32_t      grnum,   /* Tgm group number or Zero */
		      uint32_t      grval,   /* Group value if num not zero */
...
TimLibError CtrLibGet(uint32_t      ptim,
		      uint32_t      plnum,   /* Ptim line number 1..n or 0 */
		      uint32_t      grnum,
		      uint32_t      grval,
...
TimLibError CtrLibRemoteControl(uint32_t      remflg, /* 0 = Normal, 1 = Remote */
				uint32_t      module, /* The module or zero */
				uint32_t      cntr,   /* 1..8 counter number */
...
TimLibError CtrLibGetRemote(uint32_t      module,
			       uint32_t      cntr,
			       uint32_t      *remflg,
			       TimLibCcvMask *ccvm,
			       TimLibCcv     *ccv) {
...
TimLibError CtrLibGetTime(uint32_t      module, /* Module number to read from */
...
/* uint32_t      grval = TgmGetGroupValueFromTelegram(uint32_t      grnum,     */
...
TimLibError CtrLibGetTelegram(uint32_t      module,
			      TgmMachine    machine,
...
TimLibError CtrLibGetPtimObject(uint32_t      ptim, /* PTIM equipment number */
				 uint32_t      *module,
				 uint32_t      *counter,
				 uint32_t      *dimension) {
...
TimLibError CtrLibGetCtimObject(uint32_t      ctim, /* CTIM equipment number */
				 uint32_t      *eventcode) {
