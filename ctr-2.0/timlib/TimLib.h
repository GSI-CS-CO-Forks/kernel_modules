...
typedef struct {
   ...
   unsigned int  Nano;      /* Nano second in the second */
   unsigned int  CTrain;    /* Machine millisecond in cycle */
...
typedef struct {
   ...
   unsigned int     CorVer;                /* Correct VHDL/Firmware ver */
   unsigned int     ModVer[TimLibMODULES]; /* Module  VHDL/Firmware ver */
 } TimLibModuleVersion;
...
typedef struct {
   ...
   unsigned int     Ctim;       /* CTIM triggering event of this action */
   unsigned int     Payload;    /* Payload part of the trigger */
   TgmMachine       Machine;    /* Tgm machine of telegram */
   unsigned int     GrNum;      /* Telegram's group number */
   unsigned int     GrVal;      /* Telegram's group value */
 } TimLibCcv;
...
typedef struct {
   unsigned int  Valid;     /* if not zero Pll information is valid */
   unsigned int  Error;     /* Phase error */
   unsigned int  Integrator;/* Integrator value */
   unsigned int  Dac;       /* Value on DAC */
   unsigned int  LastItLen; /* Last iteration length */
   unsigned int  KI;        /* Constant of integration */
   unsigned int  KP;        /* Constant of proportionality */
   unsigned int  NumAverage;/* Numeric average */
   unsigned int  Phase;     /* Pll phase */
   float    AsPrdNs;        /* Asynchronous period */
 } TimLibPllBlock;          /* Pll status block */

typedef struct {
   unsigned int  Valid;     /* if not zero receiver information is valid */
   TimLibTime    LastRset;  /* Time when last reset */
   unsigned int  PrtyErrs;  /* Number of parity errors */
   unsigned int  SyncErrs;  /* Number of synchronization errors */
   unsigned int  CodeErrs;  /* Number of code violations */
   unsigned int  QueuErrs;  /* Number of queue overflow errors */
   unsigned int  TotlErrs;  /* Total error count */
 } TimLibRecBlock;          /* Receiver errors block */

typedef struct {
   unsigned int  Valid;     /* if not zero Card State is valid */
   TimLibCstStat Stat;      /* Full IO status */
   unsigned int  IdMSL;     /* ID chip Most significant long */
   unsigned int  IdLSL;     /* ID chip Lest significant long */
 } TimLibCstBlock;          /* Card State block */

typedef struct {
   unsigned int  Valid;            /* if not zero Board extended memory present */
   unsigned int  PllErrThresh;  /* Pll error threshold to generate a pll error RW */
   unsigned int  PllDacLowPass; /* Low Passed DAC value used if the GMT is missed (CIC filter) RO */
   unsigned int  PllDacCIConst; /* Log2 of the number of avarages of the DAC Value CIC low pass filter RW */
   unsigned int  PllMonCIConst; /* Log2 of the interrupt reduction factor (averages over this count)  RW */
   unsigned int  PllPhaseDCM;   /* (Not used yet) Phase of the spartan DLL. Not used in the standard VHDL RW */
   unsigned int  PllUtcPhasErr; /* Phase error Utc */
   unsigned int  Temperature;   /* (Not used yet) Temperature in units of 0.5 degrees C */
   unsigned int  MsMissed;      /* Number of millisencods missed since last power up. RO */
   TimLibTime    MsLastErr;     /* Time last MS was missed */
   unsigned int  PllErrCount;   /* Number of pll errors since last power up (past threshold ) RO */
   TimLibTime    PllLastErr;    /* Time of last PLL error */
   unsigned int  FrmMissed;     /* Number of missed timing frames */
   TimLibTime    FrmMissedLast; /* Time when last frame was missed */
   unsigned int  RecBadCycles;  /* Number of bad reception cycles since last power up */
   unsigned int  RecRcvdFrms;   /* Number of received frames since last power up */
   unsigned int  RecSentFrms;   /* Value of last sent frames event */
   unsigned int  PllUtcErrs;    /* Number of UTC PLL errors */
   TimLibTime    StartOne;      /* Time of the last External-1 start */
 } TimLibExtBlock;                 /* Extended memory block */

typedef struct {
   unsigned int   Module; /* Module number 1..n if 0 module stats are invalid */
...
TimLibStatus TimLibGetStatus(unsigned int  module, TimLibDevice *dev);
...
TimLibError TimLibConnect(TimLibClass   iclss,   /* Class of interrupt */
			  unsigned int  equip,   /* Equipment or hardware mask */
			  unsigned int  module); /* For HARD or CTIM classes */

/* ==================================================================== */
/* Connect to a CTIM with specified payload                             */

TimLibError TimLibConnectPayload(unsigned int  ctim,    /* The CTIM ID you want to connect to */
				 unsigned int  payload, /* The 16 bit payload in a long */
				 unsigned int  module); /* The module, or zero means don't care */
...
TimLibError TimLibQueue(unsigned int  qflag,   /* 0=>Queue, 1=>NoQueue  */
			unsigned int  tmout);  /* 0=>No time outs       */
...
unsigned int  TimLibGetQueueSize();
...
TimLibError TimLibWait(TimLibClass    *iclss,   /* Class of interrupt */
		       unsigned int   *equip,   /* PTIM CTIM or hardware mask */
		       unsigned int   *plnum,   /* Ptim line number 1..n or 0 */
		       ...
		       unsigned int   *ctim,    /* CTIM trigger equipment ID */
		       unsigned int   *payload, /* Payload of trigger event */
		       unsigned int   *module,  /* Module that interrupted */
		       unsigned int   *missed,  /* Number of missed interrupts */
		       unsigned int   *qsize,   /* Remaining interrupts on queue */
...
TimLibError TimLibSet(unsigned int  ptim,      /* PTIM to write to */
		      unsigned int  plnum,     /* Ptim line number 1..n or 0 */
		      unsigned int  grnum,     /* Group number or Zero */
		      unsigned int  grval,     /* Group value for number */
...
TimLibError TimLibGet(unsigned int  ptim,
		      unsigned int  plnum,     /* Ptim line number 1..n or 0 */
		      unsigned int  grnum,
		      unsigned int  grval,
...
TimLibError TimLibRemoteControl(unsigned int  remflg, /* 0 = Normal, 1 = Remote */
				unsigned int  module, /* Module or zero */
				unsigned int  cntr,   /* 1..8 counter number */
...
TimLibError TimLibGetRemote(unsigned int  module,
			    unsigned int  cntr,
			    unsigned int  *remflg,
			    TimLibCcvMask *ccvm,
			    TimLibCcv     *ccv);
...
TimLibError TimLibGetTime(unsigned int  module, /* Module number to read from */
...
/* long grval = TgmGetGroupValueFromTelegram(unsigned int  grnum,       */
...
TimLibError TimLibGetTelegram(unsigned int  module,
			      TgmMachine    machine,
			      TgmTelegram   *telegram);
...
TimLibError TimLibGetAllPtimObjects(unsigned int  *ptimlist,  /* List of ptim equipments */
				    unsigned int  *psize,     /* Number of ptims in list */
				    unsigned int  size);      /* Max size of list */

/* ==================================================================== */
/* Get the list of all defined CTIM objects                             */

TimLibError TimLibGetAllCtimObjects(unsigned int  *ctimlist,  /* List of ctim equipments */
				    unsigned int  *csize,     /* Number of ctims in list */
				    unsigned int  size);      /* Max size of list */

/* ==================================================================== */
/* Get the event code corresponding to a given CTIM equipment number.   */

TimLibError TimLibGetCtimObject(unsigned int  ctim, /* CTIM equipment number */
...
TimLibError TimLibCreatePtimObject(unsigned int  ptim, /* PTIM equipment number */
				   unsigned int  module,
				   unsigned int  counter,
				   unsigned int  dimension);
...
TimLibError TimLibCreateCtimObject(unsigned int  ctim, /* CTIM equipment number */
				   unsigned int  eventcode);

/* ==================================================================== */
/* Set the debug level                                                  */

void TimLibSetDebug(unsigned int  level); /* Zero means no debug */
...
TimLibError TimLibGetCableId(unsigned int  module,  /* The given module */
			     unsigned int  *cable); /* The cable ID */
...
TimLibError TimLibGetTgmInfo(TimLibTime    stamp,       /* Time you want telegram information about */
			     ...
			     unsigned int  *cytag,      /* Cycle tag */
			     unsigned int  *ncytag);    /* Next cycle tag */
...
TimLibError TimLibGetGroupValueFromStamp(TimLibTime    stamp, /* Cycle stamp  */
					 ...
					 unsigned int  next,  /* Next flag    */
					 unsigned int  *gv);  /* Group value  */
...
char *TimLibGetSpecificInfo(unsigned int  module);  /* The given module */
...
TimLibError TimLibFdQueue(int         fd,        /* File descriptor */
			unsigned int  qflag,     /* 0=>Queue, 1=>NoQueue  */
			unsigned int  tmout);    /* 0=>No time outs       */
...
TimLibError TimLibGetModuleStats(unsigned int  module,
...
TimLibError TimLibSetPllLocking(unsigned int  module,
				unsigned int  lockflag); /* 1=> Brutal, else Slow */

/* ==================================================================== */
/* Get the module that best suits a given CTIM based on the cable ID    */

unsigned int  TimLibGetModuleForCtim(unsigned int  ctim);

unsigned int  TimLibGetModuleForMch(TgmMachine mch);
