...
char *yr, *ti, *md, *mn, *dy;
   ...
   if (t->Second) {
      ...
      if (t->Nano) {
	  sprintf(&tbuf[strlen(tbuf)],".%03d",t->Nano/1000000);
      }
...
char *GetPtmName(unsigned int  eqp) {
...
unsigned int   ieqp;  /* PTIM CTIM or hardware mask */
unsigned int   ipln;  /* Ptim line number 1..n or 0 */
...
unsigned int   ictm;  /* CTIM trigger equipment ID */
unsigned int   ipld;  /* Payload of trigger event */
unsigned int   imod;  /* Module that interrupted */
unsigned int   imis;  /* Number of missed interrupts */
unsigned int   iqsz;  /* Remaining interrupts on queue */
TgmMachine     imch;  /* Corresponding TgmMachine */

TimLibCcvMask  ccvm;  /* Valid fields in ccv */
TimLibCcv      ccv;   /* CCV fields */

unsigned int  mod, pmod, chn, pdim, cnt, row, eqp, frm;
