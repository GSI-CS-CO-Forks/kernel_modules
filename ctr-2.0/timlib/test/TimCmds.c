...
char *GetTg8PtmName(unsigned int  eqp) {
...
static unsigned int  TimDebug = 0;
...
static unsigned int  QFlag = 0;
static unsigned int  TmOut = 2000;
...
unsigned int   equip, xquip, usr, gn;
unsigned int   plnum;
...
void EditCcvs(unsigned int  ptm) {

unsigned int  mod, cnt, dim;
unsigned int  pln, grn, grv;
TimLibCcvMask ccm;
TimLibCcv ccv;
...
TgvName tname;
char txt[TXT_SZ];
unsigned int  eqp, frm, mch, str, clk, mde, plw, dly, enb, pol, oms;
...
unsigned int  ptimlist[PSIZE];
int i, n, ix;
unsigned int  psize, mod, cnt, dim, nid;
...
static void EditRemote(unsigned int  mod, unsigned int  cnt) {
...
char c, *cp, *ep;
char txt[TXT_SZ];
unsigned int  str, clk, mde, plw, dly, enb, pol, oms, rfl;
...
#define CSIZE 1024
unsigned int  ctimlist[CSIZE], csize;
...
unsigned int  moad[8];
   ...
   switch (Dev) {
	 ...
	 TimLibGetHandle(&ctr);
	 ioctl(ctr,CtrIoctlGET_MODULE_DESCRIPTOR,&moad);
	 if ((moad[3] == CTRP_DEVICE_ID)
	 &&  (moad[4] == CERN_VENDOR_ID))
	    sprintf(txt,"xterm 2>/dev/null -e %s",GetFile("ctrptest"));
	 else
