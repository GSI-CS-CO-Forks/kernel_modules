 ...
 * done using sleep(1) which was far too int  and was
...
typedef struct {
   ...
   unsigned int  (*GetInstalledModuleCount)();
...
static unsigned int  CblIds[TimLibMODULES];
static unsigned int  CblVal = 0;

unsigned int  GetCableIds() {

int i;

   if (CblVal == 0) {
      bzero((void *) CblIds, sizeof(unsigned int ) * TimLibMODULES);
...
unsigned int  GetModuleForCtim(unsigned int  ctim) {
...
unsigned int  GetModuleForMch(TgmMachine mch) {
...
/* call "close(fd)" when the descriptor is no int er needed. In case of */
...
TimLibError TimLibConnectPayload(unsigned int  ctim,        /* The CTIM ID you want to connect to */
				 unsigned int  payload,     /* The 16 bit payload in a int  */
...
/*    mch:     The machine to which the interrupt beint s               */
...
TimLibError TimLibSet(unsigned int  ptim,    /* PTIM to write to */
		      unsigned int  plnum,   /* Ptim line number 1..n or 0 */
		      unsigned int  grnum,   /* Tgm group number or Zero */
		      unsigned int  grval,   /* Group value if num not zero */
...
/* unsigned int  grval = TgmGetGroupValueFromTelegram(TgmMachine    machine,   */
/*                                                    unsigned int  grnum,     */
...
TgmMachine mch;
unsigned int  secs, mlsc, nano;
int of, msk;

unsigned int  gcytag, gncytag;
   ...
   for (of=0; of<BlkEVENT_HISTORY_SIZE; of++) {
	 ...
	 if (stamp.Machine == mch) {
	    secs = (unsigned int ) TgmHisGetCycleTimeSec(his);
	    mlsc = (unsigned int ) TgmHisGetCycleTimeMSc(his);
	    nano = mlsc * 1000000;
   ...
   if (of<BlkEVENT_HISTORY_SIZE) {
	 ...
	 cyclestamp->CTrain  = 0;
      ...
      if (cytag) {
	 gcytag= (unsigned int ) TgmGetGroupNumber(mch,"CYTAG");
	 if (gcytag) *cytag  = (unsigned int ) TgmHisGetTelegram(his,gcytag);
	 else        *cytag  = 0;
      }
      if (ncytag) {
	 gncytag = (unsigned int ) TgmGetGroupNumber(mch,"NCYTAG");
	 if (gncytag) *ncytag = (unsigned int ) TgmHisGetTelegram(his,gncytag);
	 else         *ncytag = 0;
      }
      return TimLibErrorSUCCESS;
   }
...
/* with a bad value the driver will check if it beint s to your process */
