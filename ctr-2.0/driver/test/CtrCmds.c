...
char comment[128];
   ...
   if (at == Numeric) {
      arg++;
      size = v->Number;
      if (size >= CtrDrvrHISTORY_BUF_SIZE)
	 size = CtrDrvrHISTORY_BUF_SIZE -1;
   }
...
#ifdef PS_VER
      bzero((void *) comment,128);
      if ((ehsb.Entries[i].Frame.Long & 0x0f000000) == 0x03000000)
	 strcpy(comment,"Telegram frame");
      else
	 TgvFrameExplanation(ehsb.Entries[i].Frame.Long,comment);
      ctm = TgvGetMemberForFrame(ehsb.Entries[i].Frame.Long);

      printf("[%3d] Ctm:%04d/0x%08X C%04d %s ;%s\n",
	     i,
	     ctm,
	     (int) ehsb.Entries[i].Frame.Long,
	     (int) ehsb.Entries[i].CTime.CTrain,
	     TimeToStr(&(ehsb.Entries[i].CTime.Time),1),
	     comment);
#else
      printf("[%4d] 0x%08X C%04d %s\n",
	     i,
	     (int) ehsb.Entries[i].Frame.Long,
	     (int) ehsb.Entries[i].CTime.CTrain,
	     TimeToStr(&(ehsb.Entries[i].CTime.Time),1));
